...
public class PageDaoImpl extends BaseDaoImpl implements PageDao {
	...
	public Long findPageId(String titleUrl) {
		...
		if (ids.size() > 1) throw new DataIntegrityException("Se han encontrado varias paginas para el nombre especificado. La columna de base de datos deberia tener una restriccion de unicidad que no lo habria permitido.");
		if (ids.size() == 0) return null;
		Long id = ids.get(0);
		return id;
	}

	// cuenta el numero total de paginas del sistema
	@Override
	...
	// recupera todas las paginas del sistema con paginacion, ordenadas por id descendentemente
	@Override
	public List<Page> getAllPages(int firstResult, int maxResults) {
		logger.debug("Recuperando {} elementos de todas las paginas del sistema. Primer elemento: {}", maxResults, firstResult);
	...
	// inserta una pagina en la base de datos y devuelve el id generado
	@Override
	...
	// modifica una pagina en la base de datos
	@Override
...
//		flush(); // este flush deberia ir en un interceptor de AOP asociado a los servicios o a los actions
	...
	// borra una pagina de la base de datos
	@Override
