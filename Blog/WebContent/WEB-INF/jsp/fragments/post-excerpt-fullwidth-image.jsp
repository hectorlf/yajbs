<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%><%@taglib prefix="joda" uri="http://www.joda.org/joda/time/tags"%><%@taglib prefix="stripes" uri="http://stripes.sourceforge.net/stripes.tld"%><%@taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%><%@taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions"%><fmt:formatNumber minIntegerDigits="2" type="number" value="${post.month}" var="month"/><fmt:formatNumber minIntegerDigits="2" type="number" value="${post.day}" var="day"/><stripes:url beanclass="com.hectorlopezfernandez.action.ArchiveAction" var="postUrl"><stripes:param name="year" value="${post.year}"/><stripes:param name="month" value="${month}"/><stripes:param name="day" value="${day}"/><stripes:param name="name" value="${post.titleUrl}"/></stripes:url><stripes:url beanclass="com.hectorlopezfernandez.action.AuthorsAction" var="authorUrl"><stripes:param name="name" value="${post.author.username}"/></stripes:url>
			<article id="post-${post.id}" class="post sticky pw pw1 <c:choose><c:when test="${position % 2 == 1}">odd</c:when><c:otherwise>even</c:otherwise></c:choose>">
