...
<c:otherwise><c:forEach items="${actionBean.results}" var="result" varStatus="status">			<article id="result-${status.index}" class="post <c:choose><c:when test="${status.count % 2 == 1}">odd</c:when><c:otherwise>even</c:otherwise></c:choose>">
				<header>
					<hgroup><c:choose><c:when test="${result.type eq postType}"><fmt:formatNumber minIntegerDigits="2" type="number" value="${result.month}" var="month"/><stripes:url beanclass="com.hectorlopezfernandez.action.ArchiveAction" var="resultUrl"><stripes:param name="year" value="${result.year}"/><stripes:param name="month" value="${month}"/><stripes:param name="name" value="${result.titleUrl}"/></stripes:url></c:when><c:otherwise><stripes:url beanclass="com.hectorlopezfernandez.action.PagesAction" var="resultUrl"><stripes:param name="name" value="${result.titleUrl}"/></stripes:url></c:otherwise></c:choose>
